"""DECISIVE TEST: torrentium (strongest proxy signal) with TRUE graph shortest-path distance
from h20v04. If the amplification gradient survives real network distance -> the signal is real.
If flat -> the proxy signal was an artifact.
"""
...
def main():
    ...
    print(f"{SP}: {len(s)} pts | bbox lon[{bbox[0]:.1f},{bbox[2]:.1f}] lat[{bbox[1]:.1f},{bbox[3]:.1f}]")
    tt=time.time()
    ...
    print(f"  graph {G.number_of_nodes()} nodes | torrentium pts in graph: {len(s)}")
    if len(s)<100: print("too few; stop"); return
    ...
    print(f"  true distances computed ({time.time()-tt:.0f}s), finite frac={np.isfinite(D).mean():.2f}")
    ...
    print("  === TORRENTIUM, TRUE GRAPH DISTANCE ===")
    ...
    print("\n  VERDICT: rising ratio toward small km -> real amplification (proxy confirmed).")
    print("           flat/near-1 -> proxy signal was an artifact of grouping, not network distance.")
